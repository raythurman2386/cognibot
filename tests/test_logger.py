...
def wait_for_log_content(file_path, content, timeout=10):
    ...
    while True:
        if time.time() - start_time > timeout:
            raise TimeoutError(
                f"Content '{content}' was not found in file {file_path} in time."
            )
...
def wait_for_log_completion(logger, timeout=10):
    ...
    start_time = time.time()
    while any(
        isinstance(handler, AsyncRotatingFileHandler)
        and not handler.executor._work_queue.empty()
...
def test_log_rotation():
    ...
    logger = Logger(
        "RotateLogger", log_file="rotate_test.log", max_bytes=50, backup_count=2
    )
