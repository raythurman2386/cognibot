...
async def test_roll_dice_invalid_format(cog):
    ...
    ctx.respond.assert_called_once_with("❌ Invalid dice format. Use the format 'dN', where N is a number (e.g., d6, d20).")
...
async def test_roll_dice_zero_sides(cog):
    ...
    ctx.respond.assert_called_once_with("❌ The number of sides must be a positive integer.")
